...
function StudentDashboard() {
  ...
  const fetchUsername = async (email) => {
    try {
      const response = await axios.get(`https://lms1-494718652739.us-central1.run.app/username/email/${email}`);
  ...
  const fetchActiveQuizzes = async () => {
    try {
      setLoading(true);
      const response = await axios.get('https://lms1-494718652739.us-central1.run.app/active-quizzes');
  ...
  const fetchStudentProgress = async (email) => {
    try {
      ...
      const response = await axios.get(`https://lms1-494718652739.us-central1.run.app/admin/student-progress/email/${email}`, {
  ...
  const fetchQuizResults = async (quizId) => {
    try {
      setLoading(true);
      const response = await axios.get(`https://lms1-494718652739.us-central1.run.app/results/email/${studentEmail}/quizid/${quizId}`);
  ...
  const fetchLeaderboard = async (quizId) => {
    try {
      setLoading(true);
      const response = await axios.get(`https://lms1-494718652739.us-central1.run.app/leaderboard/${quizId}`);
  ...
  const checkSubmissionStatus = async (quizId) => {
    try {
      const response = await axios.get(`https://lms1-494718652739.us-central1.run.app/checkquizSubmission/${quizId}/${studentEmail}`);
...
function QuizTaker({ quiz, studentEmail, onComplete }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await axios.post('https://lms1-494718652739.us-central1.run.app/submit-quiz', {
        quizId: quiz.id,
