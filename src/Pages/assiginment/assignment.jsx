...
const Assignment = () => {
  ...
  useEffect(() => {
    ...
    const fetchUsername = async () => {
      try {
        ...
        const response = await axios.get("https://lms1-494718652739.us-central1.run.app/username", {
  ...
  const fetchCourses = async () => {
    try {
      const res = await axios.get("https://lms1-494718652739.us-central1.run.app/courses");
  ...
  const checkAdmin = async (email) => {
    try {
      const res = await axios.post("https://lms1-494718652739.us-central1.run.app/check-role", { email });
  ...
  const deleteAssignment = (courseName, assignmentName) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`https://lms1-494718652739.us-central1.run.app/admin/course/${courseName}/deleteassignment/${assignmentName}`, {
          method: 'DELETE',
        })
          .then(response => {
            if (!response.ok) {
  ...
  const selectCourse = async (course) => {
    ...
    try {
      const res = await axios.get(`https://lms1-494718652739.us-central1.run.app/students/courses/${course.name}/assignments`);
  ...
  const checkSubmissionStatus = async (courseName, assignmentName) => {
    try {
      const response = await axios.post(
        `https://lms1-494718652739.us-central1.run.app/students/${studentName}/courses/${courseName}/assignments/${assignmentName}/checksubmission`
      );
  ...
  const createAssignment = async () => {
    ...
    try {
      await axios.post(
        `https://lms1-494718652739.us-central1.run.app/admin/courses/${newAssignment.courseName}/assignments`,
        formData,
  ...
  const uploadAssignment = async (assignment) => {
    ...
    try {
      await axios.post(
        `https://lms1-494718652739.us-central1.run.app/students/${studentName}/courses/${selectedCourse.name}/assignments/${assignment.name}/upload`,
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        }
  ...
  const getSubmissions = async (assignment) => {
    try {
      const res = await axios.get(`https://lms1-494718652739.us-central1.run.app/admin/courses/${selectedCourse.name}/assignments/${assignment.name}/submissions`);
  ...
  const submitGrade = async (assignmentName, studentName) => {
    try {
      ...
      await axios.post(
        `https://lms1-494718652739.us-central1.run.app/admin/courses/${selectedCourse.name}/assignments/${assignmentName}/students/${studentName}/grade`,
      ...
      const res = await axios.get(`https://lms1-494718652739.us-central1.run.app/admin/courses/${selectedCourse.name}/assignments/${assignmentName}/submissions`);
      setSubmissions(prev => ({
        ...prev,
                ...
                {assignments.map((assignment) => (
                  <li key={assignment.name} className="assignment-item border p-4 mb-4 rounded shadow">
                    ...
                    {assignment.pdf && (
                      <button
                        onClick={() => {
                          // window.open(`https://lms1-494718652739.us-central1.run.app/uploads/courses/${selectedCourse.name}/assignments/${assignment.name}/assignment.pdf`, '_blank');
                          window.open(`https://lms1-494718652739.us-central1.run.app/downloadassignment/${selectedCourse.name}/${assignment.name}`, '_blank');
                        }}
                              ...
                              {(Array.isArray(submissions[assignment.name]) ? submissions[assignment.name] : [])
                                ...
                                .map((submission, index) => (
                                  <li key={`ungraded-${submission.student}-${index}`} className="submission-item border p-4 mb-2 rounded bg-white shadow-sm">
                                    ...
                                    {submission.filename && (
                                      <a
                                        // href={`https://lms1-494718652739.us-central1.run.app/${submission.FilePath.replace(/\\/g, "/")}`}
                                        href={`https://lms1-494718652739.us-central1.run.app/download/${submission.student}/${submission.coursename}/${submission.assignmentname}/${submission.filename}`}
                                        target="_blank"
