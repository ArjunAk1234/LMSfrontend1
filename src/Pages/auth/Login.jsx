...
//     try {
//       const res = await axios.post("https://lms1-494718652739.us-central1.run.app/login", {
...
//           "https://lms1-494718652739.us-central1.run.app/request-otp1",
//           { email: formData.email },
...
//         "https://lms1-494718652739.us-central1.run.app/verify-otp1",
//         { email, otp },
//         headers
//       );
...
//       const response = await axios.post("https://lms1-494718652739.us-central1.run.app/request-otp1", {
...
//     const response = await axios.post("https://lms1-494718652739.us-central1.run.app/forgotpassword", {
//       email: forgotPasswordEmail,
...
function LogIn() {
  ...
  async function handleLogin(event) {
    ...
    try {
      ...
      if (res.data && res.data.token) {
        ...
        // Navigate to dashboard directly without OTP
        navigate("/dashboard");
      } else {
        setError("Invalid credentials!");
      }
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      // Request password reset directly to /forgotpassword endpoint
      const response = await axios.post("https://lms1-494718652739.us-central1.run.app/forgotpassword", {
        email: forgotPasswordEmail
      });
      
      if (response.data && response.data.success) {
        setResetSuccess("Password reset email sent! Please check your inbox for further instructions.");
        
        // Clear the email field
        setForgotPasswordEmail("");
        
        // Show success message and return to login after delay
        setTimeout(() => {
          handleBackToLogin();
        }, 5000);
      } else {
        setError("Failed to process reset request. Please check your email address.");
      }
    } catch (err) {
      setError(err.response?.data?.error || "Failed to process request. Please try again.");
    } finally {
  ...
  return (
    <div className="login-container">
      <form onSubmit={showForgotPassword ? handleForgotPassword : handleLogin} className="login-form">
        ...
        <div className="form-content">
          <div>
            <h1>{showForgotPassword ? "Forgot Password" : "Log In"}</h1>
          </div>
          <hr />
          ...
          >
            {isLoading 
              ? (showForgotPassword ? "Processing..." : "Logging In...") 
        ...
        <div className="welcome-message">
          {showForgotPassword ? (
            <>
              ...
              <p>We'll help you get back into your account securely.</p>
            </>
          ) : (
            <h1>Welcome to Summer School!</h1>
          )}
