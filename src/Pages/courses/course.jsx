...
export default function CourseManagement() {
  ...
  const deleteCourse = (courseName) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`https://lms1-494718652739.us-central1.run.app/admin/deletecourse/${courseName}`, fetchOptions)
          .then(response => {
            if (!response.ok) {
  ...
  const selectCourse = async (course) => {
    ...
    try {
      const res = await axios.get(`https://lms1-494718652739.us-central1.run.app/course/${course.name}/resources`);
  ...
  const createCourse = async () => {
    try {
      await axios.post("https://lms1-494718652739.us-central1.run.app/admin/course", { name: newCourse });
      setNewCourse("");
  ...
  const uploadResource = async () => {
    ...
    try {
      await axios.post(
        `https://lms1-494718652739.us-central1.run.app/admin/course/${selectedCourse.name}/resource`,
        formData,
  ...
  const uploadNote = async () => {
    ...
    try {
      await axios.post(
        `https://lms1-494718652739.us-central1.run.app/admin/courses/${selectedCourse.name}/uploadTextNote`,
        noteData,
            ...
            {resources.length > 0 ? (
              <ul className="resource-list list-disc pl-5">
                {resources.map((res) => (
                  <li key={res}>
                    <a
                      href={`https://lms1-494718652739.us-central1.run.app/course/${selectedCourse.name}/resource/${res}`}
                      target="_blank"
