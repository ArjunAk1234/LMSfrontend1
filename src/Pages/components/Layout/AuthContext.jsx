...
export const AuthProvider = ({ children }) => {
    ...
    const checkUserRole = async (email) => {
        ...
        try {
            ...
            const roleRes = await axios.post("https://lms1-494718652739.us-central1.run.app/check-role", { email });
    ...
    const fetchProfilePhoto = async (email) => {
        try {
            ...
            if (detailsResponse.data.details && detailsResponse.data.details.photo_path) {
                ...
                const photoUrl = `https://lms1-494718652739.us-central1.run.app/uploads/${encodeURIComponent(email.replace('@', '_at_').replace('.', '_dot_'))}/photo.jpg?t=${timestamp}`;
