...
export default function UserProfile() {
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      const detailsResponse = await axios.get(`https://lms1-494718652739.us-central1.run.app/userdetails/${encodedEmail}`);
      ...
      if (detailsResponse.data.details) {
        ...
        if (details.photo_path) {
          // const photoUrl = `https://lms1-494718652739.us-central1.run.app/uploads/${encodeURIComponent(user.email.replace('@', '_at_').replace('.', '_dot_'))}/photo.jpg`;
      ...
      const usernameResponse = await axios.get(`https://lms1-494718652739.us-central1.run.app/username/email/${encodedEmail}`);
  ...
  const handleCancelEdit = () => {
    ...
    if (userDetails && userDetails.photo_path) {
      // setPhotoPreview(`https://lms1-494718652739.us-central1.run.app/uploads/${encodeURIComponent(user.email.replace('@', '_at_').replace('.', '_dot_'))}/photo.jpg`);
      setPhotoPreview(`https://lms1-494718652739.us-central1.run.app/profilepic/${encodeURIComponent(user.email)}`);
    } else {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`https://lms1-494718652739.us-central1.run.app/${endpoint}`, formDataToSend, {
        headers: {
            ...
            <div className="avatar-container">
              {userDetails && userDetails.photo_path ? (
                <div className="avatar-img">
                  <img
                    // src={`https://lms1-494718652739.us-central1.run.app/uploads/${encodeURIComponent(user.email.replace('@', '_at_').replace('.', '_dot_'))}/photo.jpg?t=${new Date().getTime()}`}
                    src={`https://lms1-494718652739.us-central1.run.app/profilepic/${encodeURIComponent(user.email)}?t=${new Date().getTime()}`}
